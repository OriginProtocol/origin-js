...
const NAMES = [
    'Stan','Micah','Tyler','Josh','Daniel','Matthew','Kay',
    'Пётр', '계룡/鷄龍', 'Urist'
]

const WORDLIST = [
    // Borrowed from https://github.com/punkave/random-words (MIT)
    // who orrowed from xkcd password generator which borrowed it from wherever
...
async function createSampleData(){
    ...
    */
    ...
    for(i in NAMES){
        ...
        let lastName = randomWord().toUpperCase()[0] + '.'
        const account = accounts[i]
        console.log(chalk`⬢  Creating user {bold.hex('#09f4a6') ${firstName} ${lastName}} ${account}`)
        o.contractService.web3.eth.defaultAccount = account
        if(Math.random()<0.66){
            console.log("    ...with profile")
            await o.users.set({profile:
                {
                    firstName: firstName,
                    lastName: lastName
                }
    ...
    for(let i = 0; i < NUMBER_OF_LISTINGS; i++){
        const brand = randomTitleWord().toUpperCase().replace(/[AEIOU]/,'')
        ...
        console.log(chalk` ⬢  Creating listing {bold.hex('#d408f4') ${listingName}} from {bold.hex('#09f4a6') ${userFirst}}`)
    ...
    for(let i = 0; i < NUMBER_OF_OFFERS; i++){
        ...
        o.contractService.web3.eth.defaultAccount = user.address
        console.log(chalk`⬢  Creating offer from {bold.hex('#09f4a6') ${userFirst}} for {bold.hex('#d408f4') ${listing.title}}`)
